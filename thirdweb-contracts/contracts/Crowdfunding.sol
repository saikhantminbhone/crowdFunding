...
contract CrowdFunding {
    ...
    function createCampaing(address _owner, string memory _title, string memory _description, uint256 _target,uint256 _deadline, string memory _image) public returns (uint256) {
        Campaign storage campain = campaigns[numberOfCampaigns];

        //check is everyting okay?
        require(campain.deadline < block.timestamp, "The Deadline should be a date in the future.");
        campain.owner = _owner;
        campain.title = _title;
        campain.description = _description;
        campain.target = _target;
        campain.deadline = _deadline;
        campain.amountCollected = 0;
